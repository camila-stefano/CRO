function injectTrendingColors() {
  const divOriginal = document.querySelector(".product-variations");
  if (!divOriginal || divOriginal.querySelector(".test-49")) return;
...
// Inicialmente
injectTrendingColors();

// Reinyecta cada vez que `.product-variations` es modificada
...
function waitForTargetAndObserve() {
  ...
  if (target) {
    ...
    // Sigue buscando el contenedor
    requestAnimationFrame(waitForTargetAndObserve);
  }
}
